...
//Cache directory
define('_THUMBNAIL_CACHE_DIR_DESC',			'Note this directory must be writable (usually chmod 777 is neccesary) for caching to work. If the directory is not writable no error will be generated but caching will be disabled.');
...
define('_THUMBNAIL_CACHE_CULLING_DESC',			'phpThumb can automatically limit the contents of the cache directory based on last-access date and/or number of files and/or total filesize.');
...
define('_THUMBNAIL_IMG_MAGIC_PATH',			'Imagemagick Path');
define('_THUMBNAIL_IMG_MAGIC_PATH_DESC',			'Windows: set absolute pathname or empty for no use of Image Magic<br />*nix: set absolute pathname to "convert", or leave as null if "convert" is in the path');
...
define('_THUMBNAIL_ERR_DIE_SOURCE_FAIL_DESC',			'die with error message if source image cannot be processed by phpThumb() (usually because source image is corrupt in some way). If false the source image will be passed through unprocessed, if true (default) an error message will be displayed.');
...
define('_THUMBNAIL_OFF_SRV_TMBN_CONF_VALID_DOMAINS_DESC',			'This is the list of domains for which thumbnails are allowed to be created. The default value of the current domain should be fine in most cases, but if neccesary you can add more domains in here, in the format www.example.com; www.example1.com.');
...
define('_THUMBNAIL_OFF_SRV_LINKING_CONF_VALID_DOMAINS_DESC',			'This is the list of domains for which thumbnails are allowed to be created. The default value of the current domain should be fine in most cases, but if neccesary you can add more domains in here, in the format www.example.com; www.example1.com.');
